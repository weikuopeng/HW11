...
_weights = {
        'wc1': tf.get_variable("wc1", [7, 7, 3, 96], initializer=tf.truncated_normal_initializer()),
        'wc2': tf.get_variable('wc2',[5, 5, 96, 256], initializer=tf.truncated_normal_initializer()),
        'wc3': tf.get_variable('wc3',[3, 3, 256, 384], initializer=tf.truncated_normal_initializer()),
        'wc4': tf.get_variable('wc4',[3, 3, 384, 384], initializer=tf.truncated_normal_initializer()),
        'wc5': tf.get_variable('wc5',[3, 3, 384, 256], initializer=tf.truncated_normal_initializer()),
        'wd2': tf.get_variable('wd2',[4096, 4096], initializer=tf.truncated_normal_initializer()),
        'out': tf.get_variable('out',[4096, n_classes], initializer=tf.truncated_normal_initializer())
    }
_biases = {
        'bc1': tf.get_variable('bc1',[96], initializer=tf.truncated_normal_initializer()),
        'bc2': tf.get_variable('bc2',[256], initializer=tf.truncated_normal_initializer()),
        'bc3': tf.get_variable('bc3',[384], initializer=tf.truncated_normal_initializer()),
        'bc4': tf.get_variable('bc4',[384], initializer=tf.truncated_normal_initializer()),
        'bc5': tf.get_variable('bc5',[256], initializer=tf.truncated_normal_initializer()),
        'bd2': tf.get_variable('db2',[4096], initializer=tf.truncated_normal_initializer()),
        'out': tf.get_variable('bout',[n_classes], initializer=tf.truncated_normal_initializer())
    }
...
def alex_net(_X, _dropout, batch_size):
    ...
    wei_den1 = tf.Variable(tf.truncated_normal([neurons, 4096], stddev=0.01))
    b_den1 = tf.Variable(tf.truncated_normal([4096], stddev=0.1))
...
def setup_inputs(sess, filenames, training_img_dir, image_size=256, crop_size=224, isTest=False, batch_size=128):
    ...
    image, y,fn = tf.train.batch([image, y, fn], batch_size=batch_size, capacity=1,name='labels_and_images')
...
with tf.variable_scope("alexnet", reuse=tf.AUTO_REUSE) as scope:
    ...
    pred2= alex_net(val_data, keep_prob,batch_size)
